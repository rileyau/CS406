        ...
        <div class='col-sm-7 col-md-8'>
            ...
            ?>
                    ...
                    <div class='pull-left comment-rating'>
                        <strong>{{$comment->totalRating()}}</strong>
                    </div>
                    @if(!Auth::guest())
                        <div class='pull-left comment-rating-buttons'>

                            @if($comment->userHasRated(Auth::user()->id) == 1)

                                {!! Form::open(['action' => ['UserCommentRatingsController@destroy', $comment->id], 'method' => 'POST', 'style' => 'display: inline;']) !!}
                                    {!! Form::hidden('_method', 'DELETE') !!}
                                ...
                                {!! Form::close() !!}
                                <br>
                                {!! Form::open(['action' => ['UserCommentRatingsController@update', $comment->id], 'method' => 'POST', 'style' => 'display: inline;']) !!}
                            ...
                            @elseif($comment->userHasRated(Auth::user()->id) == -1)
                            ...
                            @else 
                                {!! Form::open(['action' => ['UserCommentRatingsController@rate', $comment->id], 'method' => 'POST', 'style' => 'display: inline;']) !!}
                                    {!! Form::hidden('rating', 1) !!}
                                    <button type="submit" class="btn btn-default btn-xs" style='margin-bottom: 3px'>
                                        <span class="glyphicon glyphicon-chevron-up"></span>
                                    </button>
